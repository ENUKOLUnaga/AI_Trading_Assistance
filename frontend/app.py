...
if st.button("Analyze"):
    ...
    for section in required_sections:
        ...
        if section not in result:
            ...
            st.error(
                f"Missing analysis section: {section}"
            )
            st.stop()
    ...
    st.header(result["stock_name"])

    recommendation = result["recommendation_analysis"]
    ...
    with col1:
        ...
        for item in news["positive_news"]:
            st.write("•", item)
    ...
    with col2:
        ...
        for item in news["negative_news"]:
            st.write("•", item)
